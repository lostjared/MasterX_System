...
void PongWindow::drawGame(mxApp &app) {
  ...
  SDL_Rect playerPaddle = {playerPaddleX, playerPaddleY, paddleWidth,
                           paddleHeight};
  ...
  if (current - last >= 25) {
    ...
    if (ballVelX < 0 && ballX <= playerPaddleX + paddleWidth &&
    ...
    if (ballVelX > 0 && ballX + ballSize >= aiPaddleX &&
        ballX <= aiPaddleX + paddleWidth && ballY + ballSize >= aiPaddleY &&
    ...
    } else if (ballY > aiPaddleY + paddleHeight &&
               aiPaddleY < windowHeight - paddleHeight) {
      aiPaddleY += aiPaddleSpeed;
    }
    last = current;
  }

  app.printText(125, 25, "Player Score: " + std::to_string(playerScore),
                {255, 255, 255, 255});
  app.printText(350, 25, "AI Score: " + std::to_string(aiScore),
                {255, 255, 255, 255});
}
...
void PongWindow::main(mxApp &app, Dimension *dim) {
  dim_c = dim->createDimension(app, "Pong", false, false,
                               loadTexture(app, "images/pong.png"),
                               loadTexture(app, "images/pongico.png"));
  if (!dim_c) {
  ...
  pong_window = dynamic_cast<PongWindow *>(
  ...
  int windowPosX = (screenWidth - windowWidth) / 2;
  int windowPosY = (screenHeight - windowHeight) / 2;
  pong_window->create(dim_c, "Pong", windowPosX, windowPosY - 50, windowWidth,
                      windowHeight);
  ...
  pong_window->menu.addItem(
      game_id,
      pong_window->menu.addIcon(loadTexture(app, "images/pongico.png")),
      create_menu_item(
  ...
  pong_window->menu.addItem(
      ...
      create_menu_item(
          "About", [](mxApp &app, Window *win, SDL_Event &e) -> bool {
            MX_MessageBox::OkMX_MessageBox(app, win->dim, "About Pong",
                                           "Pong created by Jared Bruni");
            return true;
          }));

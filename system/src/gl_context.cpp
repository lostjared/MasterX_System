...
const char *kFSSolid = "#ifdef GL_ES\n"
                       "precision mediump float;\n"
                       "#endif\n"
                       "uniform vec4 uColor;\n"
                       "varying vec2 vUV;\n"
...
unsigned int compile(unsigned int type, const char *src) {
  ...
  if (!ok) {
    ...
    mx::system_err << "GLContext: shader compile error: " << log << "\n";
    glDeleteShader(sh);
    return 0;
  }
  return sh;
...
unsigned int link(unsigned int vs, unsigned int fs) {
  ...
  if (!ok) {
    ...
    mx::system_err << "GLContext: program link error: " << log << "\n";
    glDeleteProgram(p);
    return 0;
  }
  return p;
...
bool GLContext::init(SDL_Window *win) {
  ...
  if (!glctx_) {
    mx::system_err << "SDL_GL_CreateContext failed: " << SDL_GetError() << "\n";
    return false;
  }
  if (SDL_GL_MakeCurrent(win_, glctx_) != 0) {
    mx::system_err << "SDL_GL_MakeCurrent failed: " << SDL_GetError() << "\n";
    return false;
  }
  ...
  ready_ = true;
  mx::system_out << "MasterX GL: context ready (" << outW_ << "x" << outH_
                 << ")\n";
  return true;
}
...
void GLContext::shutdown() {
  ...
  if (progSolid_) {
    glDeleteProgram(progSolid_);
    progSolid_ = 0;
  }
  if (progTex_) {
    glDeleteProgram(progTex_);
    progTex_ = 0;
  }
  if (glctx_) {
...
void GLContext::setClipRect(const SDL_Rect *rect) {
  ...
  if (currentTargetFlipY()) {
    glScissor(rect->x, th - (rect->y + rect->h), rect->w, rect->h);
  } else {
...
int GLContext::currentTargetW() const { return target_ ? target_->w : outW_; }

int GLContext::currentTargetH() const { return target_ ? target_->h : outH_; }

bool GLContext::currentTargetFlipY() const { return target_ == nullptr; }
...
void GLContext::present() { SDL_GL_SwapWindow(win_); }
...
void GLContext::uploadProj(int locProj) {
  ...
  glUniform4f(locProj, 2.0f / static_cast<float>(tw),
              2.0f / static_cast<float>(th), yOff, flip);
}
...
void GLContext::useTextured(Texture *t) {
  ...
  if (locTexSampler_ >= 0)
    glUniform1i(locTexSampler_, 0);
  if (locTexMod_ >= 0) {
    glUniform4f(locTexMod_, t->rMod / 255.0f, t->gMod / 255.0f,
                t->bMod / 255.0f, t->aMod / 255.0f);
  }
}

void GLContext::issueQuad(float x0, float y0, float x1, float y1, float u0,
...
void GLContext::clear() {
  ...
  applyTarget();
  glClearColor(drawCol_.r / 255.0f, drawCol_.g / 255.0f, drawCol_.b / 255.0f,
               drawCol_.a / 255.0f);
...
void GLContext::fillRect(const SDL_Rect *rect) {
  ...
  useSolid();
  if (locSolidColor_ >= 0) {
    glUniform4f(locSolidColor_, drawCol_.r / 255.0f, drawCol_.g / 255.0f,
                drawCol_.b / 255.0f, drawCol_.a / 255.0f);
  }
  SDL_Rect r =
      rect ? *rect : SDL_Rect{0, 0, currentTargetW(), currentTargetH()};
  issueQuad(static_cast<float>(r.x), static_cast<float>(r.y),
            static_cast<float>(r.x + r.w), static_cast<float>(r.y + r.h), 0, 0,
            1, 1);
}
...
void GLContext::copy(Texture *t, const SDL_Rect *src, const SDL_Rect *dst) {
  ...
  useTextured(t);
  SDL_Rect d = dst ? *dst : SDL_Rect{0, 0, currentTargetW(), currentTargetH()};
  ...
  issueQuad(static_cast<float>(d.x), static_cast<float>(d.y),
            static_cast<float>(d.x + d.w), static_cast<float>(d.y + d.h), u0,
            v0, u1, v1);
...
Texture *GLContext::createFromSurface(SDL_Surface *surf) {
  ...
  if (surf->format->format != SDL_PIXELFORMAT_ABGR8888) {
    converted = SDL_ConvertSurfaceFormat(surf, SDL_PIXELFORMAT_ABGR8888, 0);
    if (!converted)
      return nullptr;
    use = converted;
  }
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, use->w, use->h, 0, GL_RGBA,
               GL_UNSIGNED_BYTE, use->pixels);
...
int RenderCopy(GLContext *r, Texture *t, const SDL_Rect *src,
               const SDL_Rect *dst) {
  if (r)
    r->copy(t, src, dst);
  return 0;
...
int QueryTexture(Texture *t, Uint32 *fmt, int *access, int *w, int *h) {
  ...
  if (access)
    *access = t && t->fbo ? SDL_TEXTUREACCESS_TARGET : SDL_TEXTUREACCESS_STATIC;
  if (w)
    *w = t ? t->w : 0;
  if (h)
    *h = t ? t->h : 0;
...
void GLContext::applyEffect(Texture *t, unsigned int prog, float time) {
  ...
  if (locRes >= 0)
    glUniform2f(locRes, static_cast<float>(tw), static_cast<float>(th));
  ...
  // Full-screen quad
  issueQuad(0.0f, 0.0f, static_cast<float>(tw), static_cast<float>(th), 0.0f,
            0.0f, 1.0f, 1.0f);
}

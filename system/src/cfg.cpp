...
Item ConfigFile::itemAtKey(const std::string &section, const std::string &key) {
  if (values[section].find(key) == values[section].end()) {
    mx::system_err << "MasterX System Error: Could not find Key: " << key
                   << " in config file.\n";
    ;
    mx::system_err.flush();
    exit(EXIT_FAILURE);
...
void ConfigFile::setItem(const std::string &section, const std::string &key,
...
ConfigFile::ConfigFile(const std::string &filex) { loadFile(filex); }

ConfigFile::~ConfigFile() { saveFile(filename); }

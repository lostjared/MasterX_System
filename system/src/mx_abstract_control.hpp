...
using EventCallback =
    std::function<bool(mxApp &app, Window *window, SDL_Event &e)>;
...
public:
  ...
  template <typename F> void setCallback(F callb) { callback = callb; }
  template <typename F> void setResizeCallback(F callb) { rcallback = callb; }

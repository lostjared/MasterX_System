...
void drawBevelFrame(SDL_Renderer *ren, const SDL_Rect &rect) {
  ...
  SDL_RenderDrawLine(ren, rect.x, rect.y + rect.h - 1, rect.x + rect.w - 1,
                     rect.y + rect.h - 1);
  SDL_RenderDrawLine(ren, rect.x + rect.w - 1, rect.y, rect.x + rect.w - 1,
                     rect.y + rect.h - 1);
  ...
  if (rect.w > 3 && rect.h > 3) {
    ...
    SDL_RenderDrawLine(ren, inner.x, inner.y + inner.h - 1,
                       inner.x + inner.w - 1, inner.y + inner.h - 1);
    SDL_RenderDrawLine(ren, inner.x + inner.w - 1, inner.y,
                       inner.x + inner.w - 1, inner.y + inner.h - 1);
  }
}
} // namespace
...
void SystemBar::addMinimizedWindow(Window *window) {
  DimensionContainer *con =
      dynamic_cast<DimensionContainer *>(dimensions->operator[](cur_dim).get());
  if (con != nullptr) {
...
void SystemBar::restoreWindow(Window *window) {
  if (window->minimized) {
    ...
    DimensionContainer *con = dynamic_cast<DimensionContainer *>(
        dimensions->operator[](cur_dim).get());
    if (con != nullptr) {
...
void SystemBar::drawMinimizedMenu(mxApp &app, int button_x, int button_y,
                                  int button_width) {
  ...
  for (int y = 0; y < menu_height; ++y) {
    ...
    SDL_Color color;
    color.r =
        gradStart.r + static_cast<Uint8>(factor * (gradEnd.r - gradStart.r));
    color.g =
        gradStart.g + static_cast<Uint8>(factor * (gradEnd.g - gradStart.g));
    color.b =
        gradStart.b + static_cast<Uint8>(factor * (gradEnd.b - gradStart.b));
    color.a = 255;

    SDL_SetRenderDrawColor(app.ren, color.r, color.g, color.b, color.a);
    SDL_RenderDrawLine(app.ren, menu_x, menu_y + y, menu_x + menu_width - 1,
                       menu_y + y);
  }
  ...
  for (size_t i = 0; i < con->mini_win.size(); ++i) {
    ...
    SDL_Surface *textSurface =
        TTF_RenderText_Blended(font, win->title.c_str(), white);
    TTF_SetFontStyle(font, TTF_STYLE_NORMAL);
    SDL_Texture *textTexture =
        SDL_CreateTextureFromSurface(app.ren, textSurface);
    int text_width = textSurface->w;
    int text_height = textSurface->h;
    SDL_FreeSurface(textSurface);
    SDL_Rect textRect = {menu_x + 30, menu_y + yOffset, text_width,
                         text_height};
    SDL_Rect icoRect = {menu_x + 5, menu_y + yOffset, 16, 16};
    SDL_RenderCopy(app.ren, textTexture, nullptr, &textRect);
    SDL_RenderCopy(app.ren, win->icon == nullptr ? app.icon : win->icon,
                   nullptr, &icoRect);
...
void SystemBar::activateDimension(int dim) {
  auto it = std::find(activeDimensionsStack.begin(),
                      activeDimensionsStack.end(), dim);
  ;
...
void SystemBar::drawDimensions(mxApp &app) {
  ...
  if (dimensions != nullptr && animationComplete == true) {
    ...
    for (size_t j = bar_offset;
         j < bar_offset + max_buttons && j < activeDimensionsStack.size();
         ++j) {

      int i = activeDimensionsStack[j];
      DimensionContainer *dim =
          dynamic_cast<DimensionContainer *>(dimensions->operator[](i).get());

      if (dim != nullptr && dim->isActive()) {

        const std::string &name = dim->name;
        SDL_Color textColor = (static_cast<int>(i) == cur_dim)
                                  ? SDL_Color{255, 255, 255, 255}
                                  : SDL_Color{0, 0, 0, 255};
        SDL_Surface *textSurface =
            TTF_RenderText_Blended(font, name.c_str(), textColor);
        ...
        bar_height = 50;
        int button_y =
            (bar_height - button_height) / 2 + app.height - bar_height;
        ...
        for (int y = buttonRect.y; y < buttonRect.y + buttonRect.h; ++y) {
          ...
          SDL_Color color;
          color.r = gradStart.r +
                    static_cast<Uint8>(factor * (gradEnd.r - gradStart.r));
          color.g = gradStart.g +
                    static_cast<Uint8>(factor * (gradEnd.g - gradStart.g));
          color.b = gradStart.b +
                    static_cast<Uint8>(factor * (gradEnd.b - gradStart.b));
          color.a = 220;

          SDL_SetRenderDrawColor(app.ren, color.r, color.g, color.b, color.a);
          SDL_RenderDrawLine(app.ren, buttonRect.x, y,
                             buttonRect.x + buttonRect.w - 1, y);
        }
        drawBevelFrame(app.ren, buttonRect);
        SDL_SetRenderDrawBlendMode(app.ren, SDL_BLENDMODE_NONE);

        SDL_Rect textRect = {button_x + 30,
        ...
        textRect.w = 20;
        textRect.h = 20;
        SDL_RenderCopy(app.ren, dim->icon == nullptr ? app.icon : dim->icon,
                       nullptr, &textRect);
        ...
        if (dim->name != "Dashboard") {
          ...
          SDL_Rect closeButtonRect = {square_x, square_y, square_size,
                                      square_size};
          if (dim->hoveringX) {
          ...
          if (xSurface != nullptr) {
            SDL_Texture *xTexture =
                SDL_CreateTextureFromSurface(app.ren, xSurface);
            int x_width = xSurface->w;
            int x_height = xSurface->h;
            SDL_FreeSurface(xSurface);

            SDL_Rect xRect = {square_x + (square_size - x_width) / 2,
                              square_y + (square_size - x_height) / 2, x_width,
                              x_height};
    ...
    if (activeIndex > 0) {
      ...
      drawArrow(app.ren, left_arrow_x, arrow_y, arrow_width, arrow_height,
                "left");
      int right_arrow_x = left_arrow_x + arrow_width + 10;
      drawArrow(app.ren, right_arrow_x, arrow_y, arrow_width, arrow_height,
                "right");
    }
    if (tcursor == true) {
      cursor_shown = true;
...
void SystemBar::drawArrow(SDL_Renderer *ren, int x, int y, int width,
                          int height, const std::string &direction) {
...
void SystemBar::setCurrentDimension(int dim) {
  if (dim != cur_dim) {
    ...
    old = dynamic_cast<DimensionContainer *>(
        dimensions->operator[](cur_dim).get());
    if (!old) {
      mx::system_err << "MasterX System: Bad cast..\n";
      mx::system_err.flush();
    ...
    setv =
        dynamic_cast<DimensionContainer *>(dimensions->operator[](dim).get());
    if (!setv) {
      mx::system_err << "MasterX System: Bad cast..\n";
      mx::system_err.flush();
      exit(EXIT_FAILURE);
    ...
    if (setv) {
      ...
      mx::system_out << "MasterX System: Switching from: " << old->name
                     << " to " << setv->name << "\n";
...
int SystemBar::getCurrentDimension() const { return cur_dim; }
...
void SystemBar::draw(mxApp &app) {
  ...
  SDL_Rect barRect = {0,
                      animationComplete == true ? app.height - barHeight : yPos,
                      windowWidth, barHeight};
  SDL_SetRenderDrawBlendMode(app.ren, SDL_BLENDMODE_BLEND);
  for (int y = 0; y < barHeight; ++y) {
    float factor = static_cast<float>(y) / barHeight;
    SDL_Color color;
    color.r = gradBarStart.r +
              static_cast<Uint8>(factor * (gradBarEnd.r - gradBarStart.r));
    color.g = gradBarStart.g +
              static_cast<Uint8>(factor * (gradBarEnd.g - gradBarStart.g));
    color.b = gradBarStart.b +
              static_cast<Uint8>(factor * (gradBarEnd.b - gradBarStart.b));
    color.a = 225;

    SDL_SetRenderDrawColor(app.ren, color.r, color.g, color.b, color.a);
    SDL_RenderDrawLine(app.ren, barRect.x, barRect.y + y,
                       barRect.x + barRect.w - 1, barRect.y + y);
  }

  SDL_Color lightBevel = {255, 255, 255, 235};
  SDL_Color darkBevel = {128, 128, 128, 235};

  SDL_SetRenderDrawColor(app.ren, lightBevel.r, lightBevel.g, lightBevel.b,
                         lightBevel.a);
  SDL_RenderDrawLine(app.ren, barRect.x, barRect.y, barRect.x + barRect.w - 1,
                     barRect.y);
  SDL_RenderDrawLine(app.ren, barRect.x, barRect.y, barRect.x,
                     barRect.y + barRect.h);

  SDL_SetRenderDrawColor(app.ren, darkBevel.r, darkBevel.g, darkBevel.b,
                         darkBevel.a);
  SDL_RenderDrawLine(app.ren, barRect.x, barRect.y + barRect.h - 1,
                     barRect.x + barRect.w - 1, barRect.y + barRect.h - 1);
  SDL_RenderDrawLine(app.ren, barRect.x + barRect.w - 1, barRect.y,
                     barRect.x + barRect.w - 1, barRect.y + barRect.h - 1);
  SDL_SetRenderDrawBlendMode(app.ren, SDL_BLENDMODE_NONE);
  ...
  SDL_Rect startButton = {windowWidth - startButtonSize,
                          animationComplete == true ? app.height - barHeight
                                                    : yPos,
                          startButtonSize, barHeight};
  ...
  if (isHovering) {
    ...
    for (int y = 0; y < startButton.h; ++y) {
      ...
      SDL_SetRenderDrawColor(app.ren, color.r, color.g, color.b, color.a);
      SDL_RenderDrawLine(app.ren, startButton.x, startButton.y + y,
                         startButton.x + startButton.w - 1, startButton.y + y);
    }
    SDL_SetRenderDrawBlendMode(app.ren, SDL_BLENDMODE_NONE);
  } else {
    SDL_Color gradStart = {192, 192, 192, 255};
...
bool SystemBar::empty() const { return activeDimensionsStack.empty(); }
...
bool SystemBar::event(mxApp &app, SDL_Event &e) {
  ...
  if (e.type == SDL_MOUSEBUTTONDOWN && e.button.button == SDL_BUTTON_LEFT) {
    ...
    if (SDL_PointInRect(&p, &arrowRectRight)) {
      ...
      int arrow_width = 15;
      int available_width =
          app.width - (arrow_width * 2) - (button_spacing * 2);
      int max_buttons = available_width / (button_width + button_spacing);
  ...
  if (showMinimizedMenu && e.type == SDL_MOUSEBUTTONDOWN &&
    ...
    if (dimensions != nullptr) {
      for (size_t j = 0; j < activeDimensionsStack.size(); ++j) {
        ...
        if (con->isActive()) {
          int button_x = 10 + activeIndex * (button_width + 10);
          activeIndex++;
          SDL_Rect closeButtonRect = {button_x + button_width - square_size - 5,
                                      button_y +
                                          (button_height - square_size) / 2,
                                      square_size, square_size};
          SDL_Point cur_point{mouseX, mouseY};
          if (con->name != "Dashboard" &&
              SDL_PointInRect(&cur_point, &closeButtonRect)) {
            con->hoveringX = true;
            cursor_shown = true;
          } else {
            con->hoveringX = false;
  ...
  if (e.type == SDL_MOUSEMOTION) {
    ...
    SDL_Rect startButtonRect = {windowWidth - startButtonSize, targetYPos,
                                startButtonSize, barHeight};
    SDL_Point mousePoint = {mouseX, mouseY};
    ...
    if (!handCur)
      handCur = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_HAND);
    if (!arrowCur)
      arrowCur = SDL_GetDefaultCursor();
    if (!arrowCur)
      arrowCur = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_ARROW);
    if (inManagedZone) {
      if (wantsHand) {
        if (handCur)
          SDL_SetCursor(handCur);
      } else {
        if (arrowCur)
          SDL_SetCursor(arrowCur);
      }
      cursor_handled = true;
    }
      ...
      for (size_t j = 0; j < activeDimensionsStack.size(); ++j) {
        ...
        if (con->isActive()) {
          ...
          if (con->name != "Dashboard" &&
            ...
            SDL_Rect buttonRect = {button_x, button_y, button_width,
                                   button_height};
...
void SystemBar::loadDimension(int id) {
  DimensionContainer *dim =
      dynamic_cast<DimensionContainer *>(dimensions->operator[](id).get());
  if (!dim) {
    mx::system_err << "MasterX System: Bad cast..\n";
    mx::system_err.flush();
    exit(EXIT_FAILURE);
  }
  if (std::find(activeDimensionsStack.begin(), activeDimensionsStack.end(),
                id) == activeDimensionsStack.end()) {
    dim->setActive(true);
...
MenuBar::~MenuBar() { TTF_CloseFont(font); }
...
void MenuBar::draw(mxApp &app) {
  ...
  SDL_SetRenderDrawColor(app.ren, hoverColor.r, hoverColor.g, hoverColor.b,
                         hoverColor.a);
  ...
  for (int i = 0; i < gradientHeight; i++) {
    ...
    SDL_RenderDrawLine(app.ren, menuX, currentY + i, menuX + menuWidth - 1,
                       currentY + i);
  }
  ...
  const char *items[] = {"Welcome",   "Terminal", "Settings", "MasterPiece",
                         "Asteroids", "Tetris",   "About",    "Shutdown"};
  ...
  for (int i = 0; i < numItems; i++) {
    itemRect = {menuX + 10, currentY + 10 + i * (itemHeight + 5),
                menuWidth - 20, itemHeight};
    ...
    SDL_Surface *textSurface =
        TTF_RenderText_Blended(font, items[i], textColor);
    SDL_Texture *textTexture =
        SDL_CreateTextureFromSurface(app.ren, textSurface);
    int textWidth = textSurface->w;
    int textHeight = textSurface->h;
    ...
    SDL_Rect textRect = {itemRect.x + (itemRect.w - textWidth) / 2,
                         itemRect.y + (itemRect.h - textHeight) / 2, textWidth,
                         textHeight};
    SDL_RenderCopy(app.ren, textTexture, nullptr, &textRect);
    SDL_DestroyTexture(textTexture);
  }
...
int MenuBar::itemClicked(mxApp &app, int x, int y) {
  ...
  SDL_Rect menuItem2 = {menuX, menuY + 10 + (itemHeight + 5), menuWidth,
                        itemHeight};
  SDL_Rect menuItem3 = {menuX, menuY + 10 + 2 * (itemHeight + 5), menuWidth,
                        itemHeight};
  SDL_Rect menuItem4 = {menuX, menuY + 10 + 3 * (itemHeight + 5), menuWidth,
                        itemHeight};
  SDL_Rect menuItem5 = {menuX, menuY + 10 + 4 * (itemHeight + 5), menuWidth,
                        itemHeight};
  SDL_Rect menuItem6 = {menuX, menuY + 10 + 5 * (itemHeight + 5), menuWidth,
                        itemHeight};
  SDL_Rect menuItem7 = {menuX, menuY + 10 + 6 * (itemHeight + 5), menuWidth,
                        itemHeight};
  SDL_Rect menuItem8 = {menuX, menuY + 10 + 7 * (itemHeight + 5), menuWidth,
                        itemHeight};

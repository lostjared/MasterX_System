...
void TextEditor::pageDown() {
  int step = std::max(1, viewRows - 1);
  cursorRow = std::min(static_cast<int>(lines.size()) - 1, cursorRow + step);
  topRow =
      std::min(std::max(0, static_cast<int>(lines.size()) - 1), topRow + step);
}

void TextEditor::fillRect(mxApp &app, const SDL_Rect &r, Uint8 r8, Uint8 g8,
                          Uint8 b8, Uint8 a8) {
...
void TextEditor::renderLine(mxApp &app, const std::string &text, int x, int y) {
  if (text.empty())
    return;
...
void TextEditor::draw(mxApp &app, const SDL_Rect &area) {
  ...
  std::string tname = filename.empty() ? std::string("[New Buffer]") : filename;
  std::string titleText =
      " MX Editor   File: " + tname + (modified ? "  [Modified]" : "");
  ...
  if (SDL_GetTicks() < statusUntil && !statusMsg.empty()) {
    renderLine(app, "[ " + statusMsg + " ]", area.x + margin, status.y + 2);
    ...
    renderLine(app, label + promptInput, area.x + margin, status.y + 2);
  } else {
    std::ostringstream oss;
    oss << "Line " << (cursorRow + 1) << "/" << lines.size() << "   Col "
        << (cursorCol + 1);
  ...
  if (prompt == PromptMode::None) {
    ...
    if ((t / 500) % 2 == 0) {
      SDL_SetRenderDrawColor(app.ren, color.r, color.g, color.b, 255);
...
bool TextEditor::event(mxApp &app, SDL_Event &e) {
  ...
  if (prompt != PromptMode::None) {
    ...
    if (e.type == SDL_KEYDOWN) {
      ...
      if (k == SDLK_ESCAPE || (k == SDLK_c && (e.key.keysym.mod & KMOD_CTRL))) {
        prompt = PromptMode::None;
        promptInput.clear();
        promptCursor = 0;
        setStatus("Cancelled");

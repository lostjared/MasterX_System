  ...
  if (font_ != nullptr && text_.length() > 0 && multi_lined == false) {
    ...
    for (int i = 0; i < static_cast<int>(multi_text.size()); ++i) {
      if (multi_text[i].length() == 0)
        continue;
      SDL_Surface *surf =
          TTF_RenderText_Blended(font_, multi_text[i].c_str(), color_);
      if (surf == nullptr) {
        mx::system_err << "MasterX System: Error creating surface.\n";
        mx::system_err.flush();
        return;
      ...
      SDL_FreeSurface(surf);
      SDL_SetRenderTarget(app.ren, app.tex);
      SDL_Rect point = {x + wx, y + wy + kTitleBarHeight + off_y + height, sw,
                        sh};
      SDL_RenderCopy(app.ren, t, nullptr, &point);
      SDL_DestroyTexture(t);
      off_y += height;
...
void Label::linkMode(bool m) { mode = m; }

void Label::create(Window *parent, const std::string &text, SDL_Color col,
                   int xx, int yy) {
  setText(text, col);
  setGeometry(xx, yy);
  this->parent = parent;
...
void Label::create_multi(Window *parent, std::vector<std::string> &t,
                         SDL_Color col, int xx, int yy) {
  setMultiLine(t);
  setGeometry(xx, yy);
  this->parent = parent;
  multi_lined = true;
...
void Button::draw(mxApp &app) {
  ...
  if (pressed) {
    SDL_SetRenderDrawColor(renderer, pressed_bg.r, pressed_bg.g, pressed_bg.b,
                           pressed_bg.a);
    topLeftColor = pressed_fg;
    bottomRightColor = {150, 150, 150, 255};
  } else if (hover) {
    SDL_SetRenderDrawColor(renderer, hover_bg.r, hover_bg.g, hover_bg.b,
                           hover_bg.a);
  } else {
  ...
  SDL_SetRenderDrawColor(renderer, topLeftColor.r, topLeftColor.g,
                         topLeftColor.b, topLeftColor.a);
  SDL_RenderDrawLine(renderer, cx, cy, cx + w - 1, cy);
  SDL_RenderDrawLine(renderer, cx, cy, cx, cy + h - 1);

  SDL_SetRenderDrawColor(renderer, bottomRightColor.r, bottomRightColor.g,
                         bottomRightColor.b, bottomRightColor.a);
  SDL_RenderDrawLine(renderer, cx + w - 1, cy, cx + w - 1, cy + h - 1);
  SDL_RenderDrawLine(renderer, cx, cy + h - 1, cx + w - 1, cy + h - 1);

  if (app.font) {
    SDL_Surface *surface =
        TTF_RenderText_Blended(app.font, text.c_str(), textColor);
    SDL_Texture *texture = SDL_CreateTextureFromSurface(renderer, surface);

    int text_width = surface->w;
    int text_height = surface->h;
    SDL_Rect dstrect = {cx + (w - text_width) / 2, cy + (h - text_height) / 2,
...
bool Button::event(mxApp &app, SDL_Event &e) {
  ...
  if (e.type == SDL_MOUSEMOTION) {
    ...
    hover =
        (mouseX >= cx && mouseX <= cx + w && mouseY >= cy && mouseY <= cy + h);
  }
  ...
  if (e.type == SDL_MOUSEBUTTONUP && e.button.button == SDL_BUTTON_LEFT) {
    int mouseX = e.button.x;
    int mouseY = e.button.y;
    if (pressed && mouseX >= cx && mouseX <= cx + w && mouseY >= cy &&
        mouseY <= cy + h) {
      pressed = false;
      if (callback && parent) {
...
void Button::create(Window *parent, const std::string &text, int x, int y,
                    int w, int h) {
  this->text = text;
  this->x = x;
  this->y = y;
...
void Image::create(mxApp &app, Window *window, const std::string &path, int x,
                   int y) {
...
void Image::resizeWindow(int w, int h) { setGeometry(x, y, w, h); }
} // namespace mx
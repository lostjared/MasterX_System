...
void MasterPiece::activate() { resetGame(); }
...
void MasterPiece::setScreen(int scr) { cur_screen = scr; }
...
bool MasterPiece::event(mxApp &app, SDL_Event &e) {
  ...
  if (cur_screen == 2 && e.type == SDL_MOUSEBUTTONDOWN &&
      e.button.button == SDL_BUTTON_LEFT) {
    x1 = e.button.x - ir.x;
    y1x = e.button.y - ir.y;
  } else if (cur_screen == 2 && e.type == SDL_MOUSEBUTTONUP &&
             e.button.button == SDL_BUTTON_LEFT) {
  ...
  case SDL_MOUSEBUTTONDOWN: {
    if (e.button.button == SDL_BUTTON_LEFT &&
        isPointInside(e.button.x, e.button.y)) {
      switch (cur_screen) {
      case 1:
        switch (cur_screen) {
        case 1: {
...
void MasterPiece::load_gfx(mxApp &app) {
  ...
  static const char *grid_block_paths[12] = {
      "images/mp_dat/block_black.png",  "images/mp_dat/block_yellow.png",
      "images/mp_dat/block_orange.png", "images/mp_dat/block_ltblue.png",
      "images/mp_dat/block_dblue.png",  "images/mp_dat/block_purple.png",
      "images/mp_dat/block_pink.png",   "images/mp_dat/block_gray.png",
      "images/mp_dat/block_red.png",    "images/mp_dat/block_green.png",
      "images/mp_dat/block_clear.png",  nullptr};
...
void MasterPiece::draw_credits(mxApp &app) {
  ...
  app.printText((640 / 2) - (200 / 2) + 15, 200, "[ Press Space to Return ]",
                color3);
...
void MasterPiece::draw_game(mxApp &app) {
    ...
    for (int j = 0; j < 8; j++) {
      if (matrix.Tiles[i][j] != BLOCK_FADE) {
        if (matrix.Tiles[i][j] != BLOCK_BLACK) {
          ...
          SDL_RenderCopy(app.ren, blocks[matrix.Tiles[i][j]], nullptr,
                         &dstRect);
        }
        x += blockWidth;
      } else {
  ...
  SDL_RenderCopy(app.ren, blocks[matrix.block.nextcolor.c1], nullptr,
                 &nextBlockRect1);

  SDL_Rect nextBlockRect2 = {bx, by + blockHeight, blockWidth, blockHeight};
  SDL_RenderCopy(app.ren, blocks[matrix.block.nextcolor.c2], nullptr,
                 &nextBlockRect2);

  SDL_Rect nextBlockRect3 = {bx, by + 2 * blockHeight, blockWidth, blockHeight};
  SDL_RenderCopy(app.ren, blocks[matrix.block.nextcolor.c3], nullptr,
                 &nextBlockRect3);
...
void MasterPiece::blockProc() {
  ...
  if (matrix.block.y + 3 < 17 &&
      matrix.Tiles[matrix.block.y + 3][matrix.block.x] != 0) {
    matrix.setblock();
    return;
  }
  ...
  for (int i = 0; i < 17; i++) {
    for (int j = 0; j < 8 - 2; j++) {
      ...
      if (cur_color != BLOCK_BLACK && cur_color != BLOCK_FADE) {
        if (matrix.Tiles[i][j + 1] == cur_color &&
  ...
  for (int z = 0; z < 8; z++) {
    for (int q = 0; q < 17 - 2; q++) {
      ...
      if (cur_color != BLOCK_BLACK && cur_color != BLOCK_FADE) {
        if (matrix.Tiles[q + 1][z] == cur_color &&
  ...
  for (int p = 0; p < 8; p++) {
    for (int w = 0; w < 17; w++) {
      ...
      if (cur_color != 0 && cur_color != BLOCK_FADE) {
        if (w + 2 < 17 && p + 2 < 8) {
          if (matrix.Tiles[w + 1][p + 1] == cur_color &&

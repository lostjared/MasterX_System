...
template <StringType String> struct Argument {
  ...
  Argument(const Argument &a)
      : arg_name{a.arg_name}, arg_letter{a.arg_letter}, arg_value{a.arg_value},
        arg_type{a.arg_type}, desc{a.desc} {}
...
template <StringType String> struct ArgumentData {
  ...
  ArgumentData(ArgumentData<String> &&a)
      : args{std::move(a.args)}, argc{a.argc} {}
...
public:
  ...
  Argz(const Argz<String> &a)
      : arg_data{a.arg_data}, arg_info{a.arg_info}, index{a.index},
        cindex{a.cindex} {}
  ...
  Argz(Argz<String> &&a)
      : arg_data{std::move(a.arg_data)}, arg_info{std::move(a.arg_info)},
        index{a.index}, cindex{a.cindex} {}
  ...
  Argz<String> &addOptionDouble(const int &code, const String &value,
  ...
  Argz<String> &addOptionDoubleValue(const int &code, const String &value,
                                     const String &description) {
    Argument<String> a{};
    a.arg_letter = code;
        ...
        if (code != -1) {
          ...
          if (pos != arg_info.end()) {
            if (pos->second.arg_type == ArgType::ARG_DOUBLE) {
              ...
              a.arg_name = name;
              if (++index < static_cast<int>(arg_data.args.size()) &&
                  arg_data.args[index][0] != '-') {
                a.arg_name = name;
                a.arg_value = arg_data.args[index];
                index++;
                return code;
              } else {
                if constexpr (std::is_same<typename String::value_type,
                                           char>::value) {
  ...
  template <typename T> void help(T &cout) {
    using char_type = typename std::decay<
        decltype(*std::declval<T>().rdbuf())>::type::char_type;
    std::vector<Argument<String>> v;
    std::vector<Argument<String>> v2;
    for (const auto &i : arg_info) {
      if (i.second.arg_type == ArgType::ARG_SINGLE ||
          i.second.arg_type == ArgType::ARG_SINGLE_VALUE)
        v.push_back(i.second);
      else if (i.second.arg_type == ArgType::ARG_DOUBLE ||
               i.second.arg_type == ArgType::ARG_DOUBLE_VALUE)
    ...
    for (auto a = farg.begin(); a != farg.end(); ++a) {
      if (a->arg_type == ArgType::ARG_SINGLE ||
          a->arg_type == ArgType::ARG_SINGLE_VALUE) {
        if constexpr (std::is_same<char_type, char>::value) {
          ...
          String item;
          item += static_cast<char_type>(a->arg_letter);
          cout << L"-" << std::setfill(L' ') << std::setw(9) << std::left
               << item << L"\t";
        ...
        if constexpr (std::is_same<char_type, char>::value) {
          cout << "--";
          cout << std::setfill(' ') << std::left << std::setw(10)
               << a->arg_name;
          cout << "\t";
          ...
          cout << L"--";
          cout << std::setfill(L' ') << std::left << std::setw(10)
               << a->arg_name;
          cout << L"\t";

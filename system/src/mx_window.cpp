...
SDL_Cursor *getResizeCursor(ResizeCursorKind k) {
  if (!g_resizeCursorsInitialized) {
    ...
    for (int i = 0; i < 4; ++i) {
      g_resizeCursors[i] =
          createCursorFromPNG(table[i].file, table[i].hotX, table[i].hotY);
...
Window::Window(mxApp &app)
    : dim(nullptr), minimized(false), dragging(false), x(0), y(0), w(0), h(0),
      dim_w(0), dim_h(0), shown(false), remove_on(false), maximized(false),
      dragOffsetX(0), dragOffsetY(0), oldX(0), oldY(0), oldW(0), oldH(0),
      is_visible(true), reload_window(false), can_resize(false),
      isMinimizing(false), minTargetX(0), minTargetY(0), minTargetW(0),
      minTargetH(0), minAnimationStep(1), restoreTotalFrames(12),
      restoreFrame(0), restoreStartX(0), restoreStartY(0), restoreStartW(0),
      restoreStartH(0), lastMaxToggleTicks_(0), originalX(0), originalY(0),
      originalWidth(0), originalHeight(0), isRestoring(false),
      restoreTargetX(0), restoreTargetY(0), restoreTargetW(0),
      restoreTargetH(0), orig_x(0), orig_y(0), icon(nullptr),
      minimizeHovered(SDL_FALSE), closeHovered(SDL_FALSE),
      maximizeHovered(SDL_FALSE), menu(app, this) {
...
void Window::create(DimensionContainer *dim, const std::string &n, const int xx,
                    const int yy, const int ww, const int hh) {
  title = n;
  x = xx;
  y = yy + 25;
  w = ww;
...
void Window::setIcon(SDL_Texture *icon) { this->icon = icon; }

void Window::destroyWindow() { dim->destroyWindow(this); }
...
void Window::draw(mxApp &app) {
  ...
  if (isRestoring || isMinimizing) {
    ...
    drawMenubar(app);
    SDL_Rect animClip = {x + 1, y + kTitleBarHeight + 1, w - 2,
                         h - kTitleBarHeight - 2};
    ...
    drawMenubar(app);
    SDL_Rect clientClip = {x + 1, y + kTitleBarHeight + 1, w - 2,
                           h - kTitleBarHeight - 2};
...
bool Window::isVisible() const { return shown; }
...
void Window::drawMenubar(mxApp &app) {
  ...
  const int minimizeBtnX =
      allowMaximizeButton ? maximizeBtnX - btnW : closeBtnX - btnW;

  // Store hit rects (full height so click targets match visual).
  minimizeButton = {minimizeBtnX, y, btnW, btnH};
  maximizeButton = allowMaximizeButton ? SDL_Rect{maximizeBtnX, y, btnW, btnH}
                                       : SDL_Rect{0, 0, 0, 0};
  ...
  if (allowMaximizeButton) {
    ...
    if (!maximized) {
      ...
      SDL_RenderDrawLine(app.ren, back.x, back.y + 1, back.x + back.w - 1,
                         back.y + 1);
      // Redraw the front square on top.
      SDL_RenderDrawRect(app.ren, &front);
      SDL_RenderDrawLine(app.ren, front.x, front.y + 1, front.x + front.w - 1,
                         front.y + 1);
    }
  }
  ...
  SDL_RenderDrawLine(app.ren, x, y + kTitleBarHeight, frameRight,
                     y + kTitleBarHeight);
  ...
  SDL_RenderDrawLine(app.ren, x + 1, y + kTitleBarHeight + 1, frameRight - 1,
                     y + kTitleBarHeight + 1);
  SDL_RenderDrawLine(app.ren, frameRight, y + kTitleBarHeight + 1, frameRight,
                     frameBottom - 1);

  SDL_SetRenderDrawColor(app.ren, 64, 64, 64, 255);
  SDL_Surface *surface =
      TTF_RenderText_Blended(app.font, title.c_str(), {255, 255, 255});
  ...
  SDL_RenderCopy(app.ren, icon == nullptr ? app.icon : icon, nullptr,
                 &iconRect);
}

void Window::show(bool b) {
  if (!b && (minimized || isMinimizing || isRestoring) && originalWidth > 0 &&
      originalHeight > 0) {
    x = originalX;
    y = originalY;
    w = originalWidth;
    ...
    if (systemBar) {
      if (auto *con = dim) {
        auto it = std::find(con->mini_win.begin(), con->mini_win.end(), this);
        if (it != con->mini_win.end())
          con->mini_win.erase(it);
      }
    }
  }
  shown = b;
...
int Window::hitTestEdges(int mx, int my) const {
  if (!can_resize || minimized || maximized || !shown)
    return 0;
  ...
  if (SDL_PointInRect(&p, &tl))
    return 1 | 4;
  if (SDL_PointInRect(&p, &tr))
    return 2 | 4;
  if (SDL_PointInRect(&p, &bl))
    return 1 | 8;
  ...
  if (SDL_PointInRect(&p, &topEdge))
    return 4;
  if (SDL_PointInRect(&p, &bottomEdge))
    return 8;
  if (SDL_PointInRect(&p, &leftEdge))
    return 1;
  if (SDL_PointInRect(&p, &rightEdge))
    return 2;
  return 0;
}
...
void Window::removeAtClose(bool value) { remove_on = value; }
...
void Window::toggleMaximize(mxApp &app) {
  ...
  if (lastMaxToggleTicks_ != 0 &&
      (now - lastMaxToggleTicks_) < kMaxToggleDebounceMs) {
    return;
  }
...
bool Window::event(mxApp &app, SDL_Event &e) {
  ...
  if (e.type == SDL_MOUSEMOTION) {
    ...
    maximizeHovered =
        (canResize() && SDL_PointInRect(&mousePoint, &maximizeButton))
            ? SDL_TRUE
            : SDL_FALSE;
    if (resizing_) {
      ...
      // Top edge stays below the menu bar (y >= kTitleBarHeight+1); bottom
      // stays above the system bar (handled via dim_h - 50, mirroring
      // maximize()).
  ...
  if (e.type == SDL_MOUSEBUTTONDOWN && e.button.button == SDL_BUTTON_LEFT) {
    ...
    if (!inWindowButtons) {
      ...
      if (edges) {
        ...
        resizeStartW_ = w;
        resizeStartH_ = h;
        resizeMouseX_ = e.button.x;
        resizeMouseY_ = e.button.y;
        return true;
      }
    }
...
void Window::setSystemBar(SystemBar *s) { systemBar = s; }
...
void Window::minimize(bool m) {
  ...
  if (!systemBar) {
    mx::system_err << "MasterX: Window " << this->title
                   << " missing system bar link\n";
    return;
  }
...
bool Window::reload() const { return reload_window; }

void Window::setReload(bool r) { reload_window = r; }
...
bool Window::canResize() const { return can_resize; }
} // namespace mx
...
void MX_MessageBox::OkMX_MessageBox(mxApp &app, DimensionContainer *dim,
                                    const std::string &title,
                                    const std::string &text) {
  dim->objects.push_back(std::make_unique<MX_MessageBox>(app));
  MX_MessageBox *msgbox = dynamic_cast<MX_MessageBox *>(
      dim->objects[dim->objects.size() - 1].get());
  if (!msgbox) {
    mx::system_err << "MasterX System: Bad cast..\n";
    mx::system_err.flush();
    exit(EXIT_FAILURE);
  ...
  int box_w = 400;
  int box_h = 175;
  msgbox->create(dim, title, (app.width / 2) - (box_w / 2),
                 (app.height / 2) - (box_h / 2), box_w, box_h);
...
void MX_MessageBox::OkCancelMX_MessageBox(mxApp &app, DimensionContainer *dim,
                                          const std::string &title,
                                          const std::string &text,
                                          EventCallbackMsg event_cb) {
  dim->objects.push_back(std::make_unique<MX_MessageBox>(app));
  MX_MessageBox *msgbox = dynamic_cast<MX_MessageBox *>(
      dim->objects[dim->objects.size() - 1].get());
  if (!msgbox) {

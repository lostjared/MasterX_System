...
SDL_Rect TerminalTabs::tabBarRect() const {
  ...
  if (bar.h > rc.h - kTitleBarHeight)
    bar.h = std::max(0, rc.h - kTitleBarHeight);
  return bar;
}
...
void TerminalTabs::drawTabBar(mxApp &app) {
  ...
  SDL_RenderDrawLine(app.ren, bar.x, bar.y + bar.h - 1, bar.x + bar.w,
                     bar.y + bar.h - 1);
  ...
  if (tabScroll_ > maxScroll)
    tabScroll_ = maxScroll;
  if (tabScroll_ < 0)
    tabScroll_ = 0;
  // Auto-scroll to keep active tab in view.
  if (active_ < tabScroll_)
    tabScroll_ = active_;
  ...
  for (int i = tabScroll_; i < n; ++i) {
    ...
    if (fnt) {
      ...
      if (shown != label)
        shown += "…";
      app.font_printText(fnt, tab.x + kTabPad, tab.y + (tab.h - th) / 2, shown,
                         col);
    }

    // Close button.
    SDL_Rect close{tab.x + tab.w - kTabCloseSize - kTabPad,
                   tab.y + (tab.h - kTabCloseSize) / 2, kTabCloseSize,
                   kTabCloseSize};
    SDL_SetRenderDrawColor(app.ren, isActive ? 70 : 110, isActive ? 70 : 110,
                           isActive ? 70 : 110, 255);
    SDL_RenderFillRect(app.ren, &close);
    SDL_SetRenderDrawColor(app.ren, 230, 230, 230, 255);
    SDL_RenderDrawLine(app.ren, close.x + 3, close.y + 3, close.x + close.w - 4,
                       close.y + close.h - 4);
    SDL_RenderDrawLine(app.ren, close.x + close.w - 4, close.y + 3, close.x + 3,
                       close.y + close.h - 4);
  ...
  auto drawArrowBtn = [&](SDL_Rect r, bool leftArrow, bool enabled) {
    SDL_SetRenderDrawColor(app.ren, enabled ? 90 : 60, enabled ? 90 : 60,
                           enabled ? 90 : 60, 255);
    SDL_RenderFillRect(app.ren, &r);
    SDL_SetRenderDrawColor(app.ren, 20, 20, 20, 255);
    SDL_RenderDrawRect(app.ren, &r);
    SDL_SetRenderDrawColor(app.ren, enabled ? 230 : 130, enabled ? 230 : 130,
                           enabled ? 230 : 130, 255);
    int cx = r.x + r.w / 2;
    int cy = r.y + r.h / 2;
  ...
  if (arrowsVisible_) {
    leftArrow_ = SDL_Rect{bar.x + kTabPad, bar.y + 2, kArrowBtnW, bar.h - 4};
    rightArrow_ =
        SDL_Rect{bar.x + bar.w - kNewTabBtnW - kTabPad - kArrowBtnW - kTabPad,
  ...
  newTabBtn_ = SDL_Rect{bar.x + bar.w - kNewTabBtnW - kTabPad, bar.y + 3,
                        kNewTabBtnW, bar.h - 6};
  SDL_SetRenderDrawColor(app.ren, 90, 90, 90, 255);
...
void TerminalTabs::draw(mxApp &app) {
  ...
  if (Terminal *t = activeTab()) {
    SDL_Rect outer;
    Window::getRect(outer);
    SDL_Rect termClip = {outer.x + 1, outer.y + kTitleBarHeight + kTabBarH,
                         outer.w - 2, outer.h - kTitleBarHeight - kTabBarH - 1};
...
bool TerminalTabs::event(mxApp &app, SDL_Event &e) {
  ...
  if (e.type == SDL_MOUSEBUTTONDOWN && e.button.button == SDL_BUTTON_LEFT) {
    ...
    auto inRect = [&](const SDL_Rect &r) {
      return r.w > 0 && r.h > 0 && mxp >= r.x && mxp < r.x + r.w &&
             myp >= r.y && myp < r.y + r.h;
    };
    // Scroll arrows.
  ...
  if (e.type == SDL_MOUSEWHEEL && arrowsVisible_) {
    ...
    SDL_Rect bar = tabBarRect();
    if (mxp >= bar.x && mxp < bar.x + bar.w && myp >= bar.y &&
        myp < bar.y + bar.h) {
      if (e.wheel.y > 0 && tabScroll_ > 0)
        tabScroll_--;
      else if (e.wheel.y < 0)
        tabScroll_++;
      return true;
  ...
  if (Terminal *t = activeTab()) {
    const bool handled = t->event(app, e);
    // Keep tab-level font size in sync with keyboard zoom done in Terminal.
    fontSize_ = t->getFontSize();
    return handled;
  }
  return false;

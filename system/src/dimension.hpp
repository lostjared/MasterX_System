...
public:
  ...
  void init(SystemBar *sysbar, const std::string &name,
...
public:
  Dimension(mxApp &app);
  virtual ~Dimension();
  DimensionContainer *createDimension(mxApp &app, const std::string &name,
  ...
  std::string selectRandomImage(std::vector<std::string> &logos,
                                std::mt19937 &gen);
  void drawIconWithText(bool underline, SDL_Renderer *renderer, TTF_Font *font,
                        const std::string &name, SDL_Texture *ico, int x,
                        int y);
  void drawIcons(SDL_Renderer *renderer, TTF_Font *font, SDL_Texture *dicon,

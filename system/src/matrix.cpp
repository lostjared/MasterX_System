...
void createMatrixRainTexture(SDL_Renderer *renderer, SDL_Texture *texture,
  ...
  int char_width = 0;
  int char_height = 0;
  TTF_SizeUTF8(font, unicodeToUTF8(hiragana_range[0]).c_str(), &char_width,
               &char_height);
  ...
  for (int col = 0; col < num_columns; ++col) {
    ...
    for (int trail_offset = 0; trail_offset < trail_lengths[col];
         ++trail_offset) {
      int row =
          static_cast<int>(fall_positions[col] - trail_offset + num_rows) %
          num_rows;
      ...
      if (char_textures.find(random_char) == char_textures.end()) {
        SDL_Surface *surface = TTF_RenderUTF8_Blended(
            font, random_char.c_str(),
            trail_offset == 0 ? bright_green : dim_green);
      ...
      SDL_Rect dst_rect = {col * char_width, row * char_height, char_width,
                           char_height};

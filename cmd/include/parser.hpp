    ...
    class Parser {
      ...
      private:
        ...
        std::shared_ptr<cmd::Node> parseStatement() {
            ...
            if ((peek().getTokenType() == types::TokenType::TT_ARG || peek().getTokenType() == types::TokenType::TT_SYM) && peek().getTokenValue().length() > 2 && peek().getTokenValue().substr(0, 2) == "--") {
